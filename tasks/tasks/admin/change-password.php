...
if(strlen($_SESSION['alogin'])==0)
    {   
		...
		date_default_timezone_set(date_default_timezone_get());// change according timezone
		$currentTime = date( 'd-m-Y h:i:s', time () );
		//echo $_SESSION['alogin'];
		if(isset($_POST['submit']))
		{
			$regno=$_SESSION['alogin'];	
			$currentpass=md5($_POST['cpass']);
			$newpass=md5($_POST['newpass']);
			$sql=mysqli_query($con,"SELECT password FROM  admin where password='$currentpass' && username='$regno'");
			$num=mysqli_fetch_array($sql);
			if($num>0)
			{
				$con=mysqli_query($con,"update `admin` set `password`='$newpass', updationDate=now() where username='$regno'");
...
<head>
    <meta charset="utf-8" />
    <title>Панель управления | Смена пароля администратора</title>
...
<?php include('includes/header.php');?>
    ...
    <div class="content-wrapper">
        <div class="container">
              <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Смена пароля администратора </h1>
                    </div>
                </div>
                <div class="row" >
